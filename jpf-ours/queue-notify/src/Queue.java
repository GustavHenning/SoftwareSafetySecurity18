...
public class Queue {
	...
	public synchronized void put(byte[] items) {
		...
		assert (p + len <= c + size) : "p + len <= c + size failed: p = " + p + ", c = " + ", len = " + len;
		for (int i = 0; i < len; i++) {
			data[p++ % size] = items[i];
		}
		assert (p <= c + size) : "p <= c + size failed: p = " + p + ", c = " + c;
	...
	private void waitForData(int len) {
		assert (len <= size);
		// This always prints 0 2 2 meaning there is never any wait
		// consumers always reach this part when len > 0
		System.out.println("--->" + c + " " + len + " " + p);
		while (c + len > p) {
			try {
				assert(false);
				wait();
		...
		assert (p >= c + len) : "p >= c + len failed: p = " + p + ", c = " + c + ", len = " + len;
		assert (p - size <= c) : "p - size <= c failed: p = " + p + ", c = " + c;
	}
